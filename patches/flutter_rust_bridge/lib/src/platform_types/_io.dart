...
class ExternalLibrary extends BaseExternalLibrary {
  ...
  factory ExternalLibrary.open(String path, {String debugInfo = ''}) {
    // iOS apps use statically linked libraries, not dynamic frameworks
    // Using DynamicLibrary.open() fails because the library is embedded in the app bundle
    // DynamicLibrary.executable() correctly references the symbols already linked into the app
    if (Platform.isIOS) {
      return ExternalLibrary._(
        ffiDynamicLibrary: ffi.DynamicLibrary.executable(),
        debugInfo: 'by executable() for iOS$debugInfo',
      );
    }

    // For all other platforms, use the standard open() method
    return ExternalLibrary._(
